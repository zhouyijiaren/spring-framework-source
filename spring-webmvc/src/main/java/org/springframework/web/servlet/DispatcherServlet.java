...
public class DispatcherServlet extends FrameworkServlet {
	...
	static {
		...
		try {
			// 如果没有找到HandlerMapping，则使用DispatherServlet同目录下的DispatherServlet.propreties配置文件中的默认HandlerMapping
			ClassPathResource resource = new ClassPathResource(DEFAULT_STRATEGIES_PATH, DispatcherServlet.class);
			// 加载DispatcherServlet.properties中的配置属性
	...
	/**
	 * 基本变量的值可以在DispatcherServlet对应的web.xml中进行修改。例如：
	 *
	 *	<servlet>
	...
	/**
	 * FlashMap管理器，支持Spring运行重定向时携带数据。
	 * 之前携带数据需要放到请求域中，此处是将数据放入到session中，重定向之后，可以从session中获取数据
	 */
	@Nullable
	...
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		// this.handlerMappings表示的是SpringMVC处理器对应的映射器
		//  可以再一个应用中有多种。比如：通过注解形式的处理器映射器，或者通过Bean声明的处理器映射器
		// 通常情况下如果只用了一种@Controller格式的处理器映射器，则改集合的size为1，具体元素类型为：RequestMappingHandlerMapping
		if (this.handlerMappings != null) {
	...
	protected void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 使用获取到的视图对象来渲染视图
			// 默认调用的是AbstractView的render来渲染视图
	...
	protected View resolveViewName(String viewName, @Nullable Map<String, Object> model,
		...
		if (this.viewResolvers != null) {
			// 如果配置了视图解析器，则使用配置的视图解析器，
			// 如果未配置，则使用默认的视图解析器。这个默认的视图解析器在DispatcherServlet.properties配置文件中.
			// 配置文件中默认的视图解析器为：InternalResourceViewResolver
